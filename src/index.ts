...
import { renderScalarHtml } from './docs/scalar';
import { getLlmsTxt } from './docs/llms';
...
// Interactive Scalar API Reference Documentation
app.get('/docs', (c) => {
  c.header('Content-Type', 'text/html; charset=utf-8');
  c.header('Access-Control-Allow-Origin', '*');
  c.header('Access-Control-Allow-Methods', 'GET, OPTIONS');
  c.header('Cache-Control', 'public, max-age=3600');
  return c.html(renderScalarHtml('/openapi.json'), 200);
});
...
// Machine-readable /llms.txt and /llm.txt manifest for AI agents and LLMs
app.get('/llms.txt', (c) => {
  const origin = new URL(c.req.url).origin;
  c.header('Access-Control-Allow-Origin', '*');
  c.header('Access-Control-Allow-Methods', 'GET, OPTIONS');
  c.header('Cache-Control', 'public, max-age=3600');
  return c.body(getLlmsTxt(origin), 200, {
...
// ---------------------------------------------------------------------------
// REST API Endpoints (/api/v1/*) mounted via thin modular routes
// ---------------------------------------------------------------------------
...
async function extractAuthenticatedUserId(
  ...
  const headerKey = c.req.header('X-API-Key') || c.req.header('x-api-key') || c.req.header('mcp-api-key');
  const queryToken = c.req.query('apiKey') || c.req.query('token');

  return resolveUserId(db, secret, {
    bearerToken,
    headerKey,
    queryToken,
  });
