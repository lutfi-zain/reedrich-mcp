...
2. Read `finance://wallets/list` for current balances.
3. Read `finance://budgets/active` for budget utilization.
4. Read `finance://debts/active` for upcoming obligations.
...
- **THEN** the server MUST return messages referencing tools `financial_summary` and resources `finance://wallets/list`, `finance://budgets/active`, and `finance://debts/active`
...
2. Read `finance://wallets/list` to assess available current balances.
3. Read `finance://debts/active` to account for outstanding debt commitments.
...
- **THEN** the server MUST return messages containing the structured reasoning chain referencing `financial_summary`, `finance://wallets/list`, and `finance://debts/active`, and instructing the agent to compute projected timeline
...
2. Read `finance://debts/active` for aggregate totals.
...
- **THEN** the server MUST return messages referencing tools `manage_debt_loan` and `financial_summary`, and resource `finance://debts/active`, with instructions to prioritize overdue debts
