...
For every MCP tool that delegates to a service function, the MCP transport adapter MUST produce output identical to the current MCP tool output. The JSON structure, field names, field order, and error messages MUST NOT change.
...
- **THEN** the response SHALL be identical in structure and content to the current implementation
